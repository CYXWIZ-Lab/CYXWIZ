...
void Theme::ApplyImNodesStyle() {
    // Guard: ImNodes may not be initialized (e.g., server GUI doesn't use node editor)
    ImNodesContext* ctx = ImNodes::GetCurrentContext();
    if (!ctx) {
        return;  // Skip styling if ImNodes isn't initialized
    }
