...
void TextEditor::Render()
{
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			// Calculate the available width for text (for word wrap)
			float wrapWidth = contentSize.x - mTextStart - scrollX;
			int wrapLineCount = 0;  // Number of visual wrap lines for this logical line
			...
			for (int i = 0; i < line.size();)
			{
				...
				// Check if we need to wrap (word wrap enabled and exceeded wrap width)
				if (mWordWrap && bufferOffset.x > wrapWidth && bufferOffset.x > 0)
				{
					// Wrap to next visual line
					bufferOffset.x = spaceSize * 2;  // Indent wrapped lines slightly
			...
			// Adjust textScreenPos for the next line to account for word wrap
			if (mWordWrap && wrapLineCount > 0)
			{
				cursorScreenPos.y += wrapLineCount * mCharAdvance.y;
			}
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
	...
	if (aChar == '\n')
	{
		...
		if (mAutoIndent && mLanguageDefinition.mAutoIndentation)
		{
			// Copy existing leading whitespace
			for (size_t it = 0; it < line.size() && isascii(line[it].mChar) && isblank(line[it].mChar); ++it)
				newLine.push_back(line[it]);

			// Smart indent: add extra indentation if line ends with ':' (Python style)
			// Check the non-whitespace content before cursor position
			if (cindex > 0)
			{
				// Find the last non-whitespace character before cursor
				int lastNonSpace = cindex - 1;
				while (lastNonSpace >= 0 && isascii(line[lastNonSpace].mChar) && isblank(line[lastNonSpace].mChar))
					--lastNonSpace;

				// If line ends with ':', '{', '[', or '(' add extra indentation
				if (lastNonSpace >= 0)
				{
					char lastChar = line[lastNonSpace].mChar;
					if (lastChar == ':' || lastChar == '{' || lastChar == '[' || lastChar == '(')
					{
						// Add a tab for indentation
