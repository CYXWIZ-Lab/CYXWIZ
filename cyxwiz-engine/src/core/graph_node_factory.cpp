// The node factory (TOFIX118 P2): every node type's pins and default
// parameters, moved from the editor so the Engine and headless hosts build
// identical graphs from a saved document.
...
MLNode CreateGraphNode(NodeType type,
    ...
    node.type = type;
    node.category = GetNodeCategoryForType(type);  // Unified Canvas Phase 1
    node.name = name;
