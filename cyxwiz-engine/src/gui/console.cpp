...
std::string FormatRuntimeLogDetails(const cyxwiz::RuntimeLogEvent& event) {
    ...
    if (event.node_id >= 0) output << "node_id=" << event.node_id << '\n';
...
std::string BuildSavedViewExpression(
    const cyxwiz::RuntimeLogInspectorCriteria& criteria) {
...
Console::Console()
    ...
    , command_service_(
          std::make_unique<cyxwiz::RuntimeConsoleCommandService>(
...
void Console::RenderLogsToolbar() {
    ...
    ShowHelpTooltip(
        "Hide retained events through the current high-water mark. "
    ...
    ShowHelpTooltip(
        "Copy the selected runtime row. You can also press Ctrl+C.",
...
void Console::RenderCommandInput() {
    ...
    if (ImGui::InputTextWithHint(
            "##input", "Enter command...", input_buf_,
            IM_ARRAYSIZE(input_buf_), input_text_flags,
            &Console::InputTextCallback, this)) {
        if (input_buf_[0]) {
...
void Console::RenderRuntimeLogExportStatus() {
    ...
    if (running) {
        ...
        ImGui::PushStyleColor(
            ImGuiCol_Text,
            success ? ImVec4(0.3f, 0.85f, 0.4f, 1.0f)
...
void Console::RenderRuntimeLogExportDialog() {
    ...
    if (!ImGui::BeginPopupModal(
            "Export runtime logs", nullptr, ImGuiWindowFlags_NoCollapse)) {
        return;
    }
    ...
    if (!selected_available) inspector_export_selected_scope_ = false;

    ImGui::Text(
        "Frozen at sequence %llu | displayed %zu | matched %zu%s",
        static_cast<unsigned long long>(
            frozen_result->query.high_water_sequence),
    ...
    int format = inspector_export_format_ ==
    ...
    ImGui::SameLine();
    ImGui::Checkbox(
        "Dataset names", &inspector_export_redaction_.dataset_names);
    ImGui::SameLine();
    ImGui::Checkbox("Query text", &inspector_export_redaction_.query_text);
    ImGui::SameLine();
    ImGui::Checkbox(
        "Python/package output", &inspector_export_redaction_.python_output);
    ...
    if (!any_redaction) {
        ImGui::PushStyleColor(
            ImGuiCol_Text, ImVec4(1.0f, 0.65f, 0.2f, 1.0f));
        ImGui::TextWrapped(
    ...
    if (preview_event && ImGui::BeginTable(
            "RuntimeLogExportPreview", 2,
    ...
    if (ImGui::Button("Save export...")) {
        const bool json_lines = inspector_export_format_ ==
            cyxwiz::RuntimeLogExportFormat::JsonLines;
        const auto filters = json_lines
...
void Console::RenderInspectorTab() {
    const auto result = SnapshotInspectorResult();
    RenderInspectorFilters(result.get());
    ...
    ImGui::TextDisabled(
        "Showing %zu / matched %zu | retained %zu / %zu | evicted %llu | high-water %llu%s",
    ...
    if (has_selection) {
        ...
        if (ImGui::BeginTable(
                "RuntimeLogInspectorSplit", 2,
                ImGuiTableFlags_Resizable | ImGuiTableFlags_BordersInnerV)) {
            ImGui::TableSetupColumn(
                "Rows", ImGuiTableColumnFlags_WidthStretch);
            ImGui::TableSetupColumn(
                "Details", ImGuiTableColumnFlags_WidthFixed, details_width);
            ImGui::TableNextRow();
            ImGui::TableSetColumnIndex(0);
...
void Console::RenderInspectorFilters(
    ...
    for (size_t index = 0; index < inspector_criteria_.levels.size(); ++index) {
        ...
        changed |= ImGui::Checkbox(
            level_labels[index], &inspector_criteria_.levels[index]);
    ...
    ShowHelpTooltip(
        inspector_paused_
            ...
            : "Freeze this view at its current high-water mark while ingestion continues.");
    ImGui::SameLine();
    ...
    ShowHelpTooltip(
        "Show retained events hidden by Clear Log View. Store eviction still applies.");
    ...
    const std::string filters_label = field_filter_count == 0
        ? "Filters"
        : "Filters (" + std::to_string(field_filter_count) + ')';
    const float trailing_buttons_width =
        ImGui::CalcTextSize("?").x + ImGui::CalcTextSize(filters_label.c_str()).x +
        ImGui::GetStyle().FramePadding.x * 4.0f +
        ImGui::GetStyle().ItemSpacing.x * 2.0f;
    ImGui::SetNextItemWidth(std::max(
        120.0f, ImGui::GetContentRegionAvail().x - trailing_buttons_width));
    if (ImGui::InputTextWithHint(
            "##runtime_filter", "Structured filter...", inspector_filter_,
    ...
    ShowHelpTooltip(
        "Open category, source, code, run, backend, task, device, and saved-filter controls.");
    ...
    if (ImGui::BeginPopup("RuntimeLogAdvancedFilters")) {
        ...
        if (result && ImGui::BeginTable(
                "RuntimeLogFieldFilters", 2,
                ImGuiTableFlags_SizingStretchSame)) {
            const auto string_combo = [&changed](
                const char* label, const char* id, std::string& selected,
                const std::vector<std::string>& values) {
                ImGui::TableNextColumn();
                ImGui::TextDisabled("%s", label);
                ImGui::SetNextItemWidth(-1.0f);
                if (!ImGui::BeginCombo(
                        id, selected.empty() ? "Any" : selected.c_str())) {
                    return;
                }
            ...
            string_combo("Category", "##FilterCategory",
                         inspector_criteria_.category, facets.categories);
            string_combo("Source", "##FilterSource",
                         inspector_criteria_.source, facets.sources);
            string_combo("Code", "##FilterCode", inspector_criteria_.code,
                         facets.codes);
            string_combo("Run", "##FilterRun", inspector_criteria_.run_id,
                         facets.run_ids);
            string_combo("Backend", "##FilterBackend",
                         inspector_criteria_.backend, facets.backends);

            ImGui::TableNextColumn();
            ImGui::TextDisabled("Task");
            ImGui::SetNextItemWidth(-1.0f);
            const std::string task_preview = inspector_criteria_.task_id
            ...
            if (ImGui::BeginCombo("##FilterTask", task_preview.c_str())) {
                ...
                for (const auto value : facets.task_ids) {
                    ...
                    if (ImGui::Selectable(
                            label.c_str(), inspector_criteria_.task_id == value)) {
        ...
        ImGui::TextDisabled(
            "A saved view captures search, severity, fields, and structured filter.");
        ...
        if (has_saved_selection) {
            ...
            if (BuildSavedViewExpression(inspector_criteria_) !=
                saved.expression) {
        ...
        ImGui::SetNextItemWidth(-1.0f);
        if (ImGui::BeginCombo(
                "##SavedRuntimeLogView", saved_preview.c_str())) {
            if (ImGui::Selectable("Custom", !has_saved_selection)) {
                inspector_selected_saved_filter_ = -1;
                inspector_filter_name_[0] = '\0';
                inspector_filter_status_.clear();
            }
            for (size_t index = 0; index < inspector_saved_filters_.size();
                 ++index) {
                ...
                const std::string label = saved.validation_error.empty()
                    ...
                    : saved.name + " (invalid)";
                if (ImGui::Selectable(
                        label.c_str(), inspector_selected_saved_filter_ ==
                    ...
                    inspector_filter_name_[0] = '\0';
                    inspector_filter_status_ =
                        "Applied saved view '" + saved.name + "'";
        ...
        const auto validate_current = [this, &set_status](
        ...
        if (ImGui::Button("Save As")) {
            ...
            const auto existing = std::find_if(
                inspector_saved_filters_.begin(),
                inspector_saved_filters_.end(),
                [&name](const auto& saved) { return saved.name == name; });
            if (!IsValidSavedViewName(name)) {
                set_status(
                    "View name must use letters, digits, '_' or '-' (max 63).",
                    true);
            } else if (existing != inspector_saved_filters_.end()) {
                set_status(
                    "That view already exists. Select it and use Update.", true);
                ...
                inspector_selected_saved_filter_ = static_cast<int>(
                    inspector_saved_filters_.size() - 1);
        ...
        if (ImGui::Button("Update") && has_saved_selection) {
            ...
            if (validate_current(expression)) {
                auto& saved = inspector_saved_filters_[
                    inspector_selected_saved_filter_];
        ...
        if (ImGui::Button("Delete") && has_saved_selection) {
            const std::string name = inspector_saved_filters_[
                inspector_selected_saved_filter_].name;
            inspector_saved_filters_.erase(
                inspector_saved_filters_.begin() +
                inspector_selected_saved_filter_);
            inspector_selected_saved_filter_ = -1;
            inspector_filter_name_[0] = '\0';
    ...
    if (changed) RequestInspectorQuery(true);
}
        ...
        while (clipper.Step()) {
            for (int row = clipper.DisplayStart; row < clipper.DisplayEnd;
                ...
                const auto level_name =
                    cyxwiz::RuntimeLogLevelName(event.level);
                ImGui::TextUnformatted(
                    level_name.data(), level_name.data() + level_name.size());
                ...
                if (!event.backend.empty() || event.device_id >= 0) {
                    ImGui::Text("%s:%d", event.backend.c_str(),
                                event.device_id);
                }
...
void Console::RenderInspectorDetails(
    ...
    ImGui::InputTextMultiline(
        "##RuntimeLogSelectedDetails", details.data(), details.size() + 1,
        ImVec2(-1.0f, -1.0f),
...
void Console::RenderAllTab() {
    ...
    for (const auto& entry : entries) {
        ...
        if (ImGui::Selectable(
                buf, selected, ImGuiSelectableFlags_AllowDoubleClick)) {
    ...
    inspector_worker_ = std::thread([this]() {
        for (;;) {
            ...
            auto result = std::make_shared<cyxwiz::RuntimeLogInspectorResult>(
                cyxwiz::QueryRuntimeLogInspector(
                    cyxwiz::RuntimeLogStore::Instance(), request));
            inspector_query_executions_.fetch_add(
                1, std::memory_order_relaxed);
            {
                ...
                if (generation == inspector_request_generation_) {
                    ...
                    inspector_query_stale_.fetch_add(
                        1, std::memory_order_relaxed);
                }
            }
        }
...
void Console::StopInspectorWorker() {
    ...
    if (inspector_worker_.joinable()) inspector_worker_.join();
    spdlog::debug(
...
void Console::RequestInspectorQuery(bool force) {
    const auto newest =
        cyxwiz::RuntimeLogStore::Instance().GetStats().newest_sequence;
    const uint64_t through = inspector_paused_
        ? inspector_frozen_sequence_
        : newest;
    ...
    request.display_limit = 1000;
    {
        ...
        if (inspector_request_pending_) {
            inspector_query_coalesced_.fetch_add(
                1, std::memory_order_relaxed);
        }
...
void Console::LoadSavedInspectorFilters() {
    ...
    for (const auto& stored : cyxwiz::core::EngineConfig::Instance()
                                  .GetRuntimeLogSavedFilters()) {
        ...
        if (!valid_name) {
            ...
            const auto parsed =
                cyxwiz::ParseRuntimeLogFilter(saved.expression);
            if (!parsed.Ok()) {
                saved.validation_error = parsed.error
...
void Console::ExecutePipCommand(
    const std::vector<std::string>& pip_arguments) {
...
void Console::QueueRuntimeLogExport(
    const std::filesystem::path& destination) {
    const auto frozen_result = inspector_export_result_;
    if (!frozen_result || destination.empty()) return;

    auto output_path = destination;
    if (!output_path.has_extension()) {
        output_path += inspector_export_format_ ==
                cyxwiz::RuntimeLogExportFormat::JsonLines
            ? ".jsonl"
            : ".txt";
    }

    const auto selected_sequence = inspector_export_selected_scope_
         ...
         output_path, task_state](cyxwiz::LambdaTask& task) {
            try {
            ...
            } catch (const std::exception& error) {
                {
                    ...
                    task_state->success = false;
                    task_state->message =
                        std::string("Runtime-log export failed: ") +
                        error.what();
                }
                throw;
            }
