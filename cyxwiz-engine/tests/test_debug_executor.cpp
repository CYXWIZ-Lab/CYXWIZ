...
void TestNormalizationRegularizationConfigurationContract() {
    ...
    ExpectTrue(!built.ok() &&
                   built.error_message.find("[0, 1)") != std::string::npos,
               "Dropout rate=1 should fail before module construction");
    ...
    bool direct_dropout_rejected = false;
    try {
        DropoutModule invalid(1.0f);
        (void)invalid;
    } catch (const std::invalid_argument&) {
        direct_dropout_rejected = true;
    }
    ExpectTrue(direct_dropout_rejected,
               "direct DropoutModule construction should reject p=1");
