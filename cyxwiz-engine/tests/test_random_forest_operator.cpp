#include "../src/core/node_executors/random_forest_operator.h"
...
void TestNumericLabels() {
    cyxwiz::RandomForestClassifierOperator op;
    std::string error;
    Check(op.Configure({
        {"target_col", "label"},
        {"feature_cols", "x,z"},
    ...
    Check(ReadDoubleValue(output, "rf_pred", 0) == 0.0,
...
void TestStringLabels() {
    ...
    Check(result.ok(), result.status().ToString());
    auto output = result.ValueOrDie();
    Check(ReadStringValue(output, "rf_class", 0) == "low",
          "first string prediction should match low class");
    Check(ReadStringValue(output, "rf_class", 7) == "high",
...
void TestValidation() {
    ...
    Check(!op.Configure({
        {"target_col", "label"},
        {"max_features", "half"},
    }, error), "bad max_features should fail configure");
    Check(error.find("max_features") != std::string::npos,
          "bad max_features error should name max_features");
}
...
int main() {
    ...
    TestStringLabels();
    TestValidation();
    std::cout << "Random forest operator passed\n";
    return 0;
}
