...
- Consumes `fixtures/text_vectorizers_sklearn.json`, generated by
  `reference/generate_text_vectorizer_fixtures.py` with scikit-learn 1.8.
- Verifies CountVectorizer raw counts, binary presence, optional L2
  normalization, deterministic vocabulary order, and corpus-count-based
  `max_features` capping.
- Verifies TF-IDF raw count times IDF under `norm=none`, disabled-IDF behavior,
  smoothed IDF weights, L2 normalization, unigram/bigram/trigram materialization,
  and fitted vocabulary/weight state.
- Uses simple lowercase word fixtures matching the shared CyxWiz/sklearn
  tokenization subset. Stop-word-list parity is covered separately because the
  engine owns its explicit English list.
...
```powershell
cmake --build build --config Debug --target test_computation_truth_text_vectorizers -- /m:4 /v:minimal
build\bin\Debug\test_computation_truth_text_vectorizers.exe
