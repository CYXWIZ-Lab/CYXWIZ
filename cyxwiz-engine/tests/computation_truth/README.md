...
CPU compatibility fallback and process-isolated oneAPI qualification remain
explicit gaps.
...
strict ArrayFire CPU/CUDA/OpenCL residency rules apply; non-ArrayFire-build CPU
and oneAPI route qualification remain explicit gaps.
...
undeclared host synchronization. PReLU remains partial because its learned
alpha is not yet owned by a production Module/optimizer update lifecycle, the
non-ArrayFire implementation supports shared alpha only, and oneAPI execution
remains quarantined.
