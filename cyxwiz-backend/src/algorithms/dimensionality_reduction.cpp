...
static bool CheckGPUAvailable() {
    ...
    try {
        ...
        if (s_use_gpu) {
            spdlog::info("[DimensionalityReduction] GPU acceleration enabled");
        }
    } catch (const af::exception& e) {
        spdlog::warn("[DimensionalityReduction] GPU check failed: {}", e.what());
        s_use_gpu = false;
    }
#endif
    return s_use_gpu;
    ...
    const std::vector<std::vector<double>>& data)
{
...
#ifdef CYXWIZ_HAS_ARRAYFIRE
    if (CheckGPUAvailable() && n_samples > 100 && n_features > 10) {
        try {
            // Convert to flat array (column-major for ArrayFire)
            std::vector<double> flat_data;
            flat_data.reserve(n_samples * n_features);
            for (size_t j = 0; j < n_features; j++) {
                for (size_t i = 0; i < n_samples; i++) {
            ...
            af::array gpu_data(n_samples, n_features, flat_data.data());
            ...
            for (size_t i = 0; i < n_features; i++) {
                for (size_t j = 0; j < n_features; j++) {
                    cov[i][j] = cov_flat[j * n_features + i];  // Column-major
                }
            }
    ...
    // CPU fallback - Initialize covariance matrix
...
#ifdef CYXWIZ_HAS_ARRAYFIRE
    if (CheckGPUAvailable() && n > 100 && !data.empty()) {
        try {
            size_t d = data[0].size();

            // Convert to flat array (column-major)
            std::vector<double> flat_data;
            flat_data.reserve(n * d);
            ...
            for (size_t i = 0; i < n; i++) {
                for (size_t j = 0; j < n; j++) {
                    distances[i][j] = dist_flat[j * n + i];  // Column-major
                }
            }
    ...
    double min_dist)
{
    ...
    double learning_rate_opt = 1.0;
    int n_epochs = 200;

    for (int epoch = 0; epoch < n_epochs; epoch++) {
        learning_rate_opt = 1.0 * (1.0 - static_cast<double>(epoch) / n_epochs);
        ...
        for (size_t i = 0; i < n_samples; i++) {
            ...
            for (int k = 0; k < n_neighbors; k++) {
                ...
                for (int d = 0; d < n_dims; d++) {
                    double diff = result.embeddings[i][d] - result.embeddings[j][d];
                    result.embeddings[i][d] -= learning_rate_opt * w * grad_coeff * diff;
                }
            }
            ...
            for (int neg = 0; neg < 5; neg++) {
                ...
                for (int d = 0; d < n_dims; d++) {
                    double diff = result.embeddings[i][d] - result.embeddings[j][d];
                    result.embeddings[i][d] -= learning_rate_opt * (1.0 - weights[i][j]) * grad_coeff * diff;
                }
            }
        }
    }
