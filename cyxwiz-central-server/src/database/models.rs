...
// Node device model - individual GPU/CPU device within a node
...
pub struct NodeDevice {
    ...
    pub device_type: String,       // "cuda", "opencl", "cpu"
    pub device_index: i32,
    pub device_name: String,

    // Allocation settings (user-specified in GUI)
    pub is_enabled: bool,
    pub vram_allocated_mb: i64,
...
impl Node {
    ...
    pub fn can_handle_job(&self, job: &Job) -> bool {
        ...
        debug!(
            "Checking node {} for job {}: job.required_gpu={}, node.gpu_model={:?}, job.required_gpu_memory={:?}, node.gpu_memory={:?}, job.required_ram={}, node.ram={}",
            self.id, job.id, job.required_gpu, self.gpu_model, job.required_gpu_memory_gb, self.gpu_memory_gb, job.required_ram_gb, self.ram_gb
        );

        if job.required_gpu && self.gpu_model.is_none() {
            debug!("Node {} rejected: job requires GPU but node has no GPU", self.id);
            return false;
        }
        ...
        if let (Some(required_vram), Some(available_vram)) =
            ...
            if required_vram > available_vram {
                debug!("Node {} rejected: job requires {} GB VRAM but node has {} GB", self.id, required_vram, available_vram);
                return false;
            }
        }

        if job.required_ram_gb > self.ram_gb {
            debug!("Node {} rejected: job requires {} GB RAM but node has {} GB", self.id, job.required_ram_gb, self.ram_gb);
            return false;
        }

        debug!("Node {} CAN handle job {}", self.id, job.id);
        true
    }
