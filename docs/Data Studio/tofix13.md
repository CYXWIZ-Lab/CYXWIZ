...
Start with tensor parity. The tensor gap is the base blocker for almost
every higher-level algorithm. After tensor parity is tested, split the
monolithic layer implementation into the reference-style layout while
preserving public class names and Python binding behavior.
...
The tensor parity work described in this file is complete. The C++
backend tensor API now covers the required parity operations, the API
has been split into focused tensor translation units, Python bindings
expose the tensor surface, Studio exposes the tensor nodes as concrete
frontend nodes, and PyCyxWiz code export can generate graph-aware tensor
operation code.

Remaining local Studio C++ runtime execution for arbitrary tensor graph
nodes is not part of this closed parity item. That work has been carried
forward into `tofix14.md` as a separate runtime adapter concern.
